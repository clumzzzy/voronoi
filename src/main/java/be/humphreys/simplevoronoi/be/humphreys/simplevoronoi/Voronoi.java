...
public class Voronoi
{
    ...
    private Point relaxPointByLloyd(ArrayList<GraphEdge> edges) {
        double sumX = 0;
        double sumY = 0;
        HashSet<ArrayList<Double>> para = new HashSet<>();
        ...
        for (ArrayList<Double> d : para) {
            sumX += d.get(0);
            sumY += d.get(1);
        }

        Point p = new Point(sumX/edges.size(), sumY/edges.size());
    ...
    public ArrayList<Point> getRelaxByLloyd(List<GraphEdge> voronoiList, int numSites) {
        ArrayList<GraphEdge> siteEdge = new ArrayList<>();
        ArrayList<Point> result = new ArrayList<>();
        for (int i = 0; i < numSites; i++) {
            for (GraphEdge ge : voronoiList) {
                if (ge.site1 == i || ge.site2 == i) siteEdge.add(ge);
            }
            Point p = relaxPointByLloyd(siteEdge);
            result.add(p);
            siteEdge.clear();
        }
        return result;
    ...
    public static void main(String[] args) {
        ...
        double[] arrX = { 430, 680 , 900 , 486 , 735 , 950 , 535 , 770 , 362 , 550 , 990 };
        double[] arrY = { 200, 200 , 290 , 330 , 395 , 450 , 472 , 573 , 556 , 650 , 634 };
        List<GraphEdge> ge = v.generateVoronoi(
                        arrX,
                        arrY,
                    0,
                    1075,
        ...
        ArrayList<Point> arrXY = v.getRelaxByLloyd(ge, arrX.length);
        System.out.println(arrXY);
        ...
        class DrawingComponent extends JPanel {
            ...
            protected void paintComponent(Graphics gh) {
                ...
                for (GraphEdge gg: ge) {
                    ...
                    //if (gg.site1==1 || gg.site2 == 1) drp.setColor(Color.MAGENTA);
                    drp.drawLine((int) gg.x1, (int) gg.y1 , (int) gg.x2, (int) gg.y2);
                }
                drp.setColor(Color.BLACK);
                for (int i = 0; i < arrX.length; i++) {
                    drp.drawRect((int) arrX[i], (int) arrY[i], 2, 2);
                }
                drp.setColor(Color.red);
                for (Point p : arrXY) {
                    drp.drawRect((int) p.x, (int) p.y, 2, 2);
                }
            }
        }
